...
public class PlayerAnim3 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.J) && !isAttacking && !isJumping)
        {
            ...
            if (playerAnimator.GetCurrentAnimatorStateInfo(0).IsName("Run3"))
            {
                Attack();
            }
    ...
    private IEnumerator ReturnToRunning(float attackAnimationLength)
    {
        ...
        yield return new WaitForSeconds(attackAnimationLength - 0.7f);
